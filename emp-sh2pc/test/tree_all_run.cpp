...
int main(int argc, char** argv) {
  ...
  string fileNameOut = "./resultsEMPGCP/"+p_string+"tree"+fileName_real+","+t_string+","+eps_string+","+N_string+","+sortOption_string+";"+fileNameOutIndex+".txt";
...
//    originalDataEncodedNot.erase (i);
   ...
   // metricRunTimeDPMerge[i] = durationDPMerge.count();
    ...
    for (int j = 0; j < int(intervalss.size()); j++) { 
      PdpI_ = addTwoVectors(PdpI_, dpHists[intervalss[j]]);
    }
    std::vector<int> RdpI;
    int idx = 0;
    for (int j = 0; j < bins; j++) {   
